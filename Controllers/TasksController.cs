...
public class TasksController : BaseController
{
    ...
    public async Task<IActionResult> Schedule(int id, string? start, int? duration, int? employeeId)
    {
        ...
        var ok = await _tasks.ScheduleAsync(id, changeStart, when, duration, CurrentUserId, IsAdmin, employeeId);
    ...
    public async Task<IActionResult> Create(TaskFormViewModel model)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            if (error != null)
            {
                ModelState.AddModelError(nameof(model.AssigneeEmail), error);
            }
